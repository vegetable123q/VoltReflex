...
│   ├── agents.py             # Agent 实现 (Rule, LLM, CoT, MetaReflexion)
...
python main.py --days 14 --agents cot         # 自适应 CoT Agent (推荐)
python main.py --days 14 --agents meta        # Meta-Reflexion Agent (AGA 架构)

# 运行所有 LLM Agent 对比
python main.py --days 14 --agents rule simple_llm cot meta

# 运行 AGA 训练脚本 (专用于 MetaReflexionAgent)
...
无记忆的 LLM Agent，每次决策独立调用 LLM。表现较差，常常忽略物理约束。

### 3. CoTAgent (自适应 Chain-of-Thought) ⭐ 核心改进
自适应 CoT Agent，具有自主学习能力：

**核心特点**：
- **动态阈值学习**: 观察历史价格自动判断高/低价，无需人工设定阈值
...
**工作流程**：
1. 收集价格历史 → 计算统计信息
2. 提供上下文给 LLM → 让其自主判断价格高低
3. 每日结束反思 → 提取优化后的阈值
4. 下一天使用学习到的阈值指导决策

### 4. MetaReflexionAgent (Agent-Generates-Agent) 🚀 最新架构
采用 AGA 架构的元反思 Agent，不再输出文本策略，而是生成 Python 代码：

**核心创新**：
- **代码即策略**: LLM 生成完整 Python 代码，不是自然语言
- **自动调试**: 编译错误会反馈给 LLM 自动修复
- **沙箱执行**: 生成的代码在隔离环境中安全执行
- **策略迭代**: 每天根据收益反馈优化代码逻辑
- **最佳保存**: 自动保存表现最好的策略代码

**工作流程**：
1. LLM 生成 Python 策略代码
2. StrategyLoader 加载并验证代码
3. 在环境中执行生成的策略
4. 收集性能数据 (profit, actions)
5. 反馈给 LLM 生成改进版本
